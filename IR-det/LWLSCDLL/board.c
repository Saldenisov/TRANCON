/*  Board.C				PCI Version       V 2.228
  ...
  on Win95:		LscPCI.vxd
	...
	V2.6	with FIFO version included
	V2.8	with  cooler functions
	V2.9	with trigger FF for short pulses
	V2.91	delay for FFT<3mu sec
	V2.92	VCLKREG for FIFO FFT
	V2.93	VFREQ for not FIFO in ns
	V2.95	resort IR FIFO for 2nd channel, TG for 2nd channel
	V2.96	ring buffer for multi processor tasks + setup ccdfkt in readfifo
	V2.97	insert external Opto Input state in data stream at pixel=1 DB7	
	V2.98	keep max. accumulated lines for ring puffer GetLastMaxLines
	V2.99	bug in Cal16Bit for FIFO
	V2.100	16 bit IR resort
	V2.101  opstate für 12 bit (pixel-2)
	V2.101	Flag HA_IR
	V2.102  SetTemp(ULONG)
	V2.103  check driver buffer size
	V2.104  writing to 2 windows parallel if HWCH2
	V2.105  DAT (delay after trigger) in S0+0x20
	V2.200	with ring buffer for multi core parallel thread FIFO and noFIFO
	V2.201 	added DELAY for WRFIFO in SetBoardVars
	V2.202	GetNumofProcessors added
	V2.204	PickOneFifoScan changed
	V2.205	IFC Control if EC=0 , Error in SendCommand (ND CLK)
	V2.206	RingReadThread with dynamic ring memory depth
	V2.207b	new: SetTORReg, CntBoards
	V2.208	TempGood for 4 channels
	V2.209	Cal16bit via Sendcommand & SetArray & err in TG(3,4)
	V2.210	with IR SetArray and new start of ReadRingThread
	V2.211	with SetupIR, changed SetTORReg
	V2.212	IR Area: RSArray
	V2.214	_OLDCAL & _FFPCI133
	V2.215  compatible to PCI P123.7
	V2.216  CallIoRead correct sort for FFT and ADS8422 
	V2.217  Cal16Bit changed to CAL_AD
	V2.219  SetAndantFSYNC
	V2.220  SetISFFT and SetISPDA for new flags in 123.8 for EC control
	V2.221  SetArray calls RSArray
	V2.222  SetIRAndant corrected
	V2.223  new function SendAndantROI 
	V2.224  volatile ThreadOn
	V2.225	Ringdrvno=DRV;
	V2.226  _TESTRUP for Andanta Sensor
	V2.227	new _HILO resort for ADS8422
	V2.228  SetROILines
...
BOOL CCDDrvInit(UINT drvno)
{
	...
	sprintf(AString,"%s%u",DRIVERNAME,drvno);
...
{	//pportadr, pclk, xckdelay for compatibility - no function here
	...
	//if (flag816==1)  
		buswidth=0x0840;//8 and 12 bit allways type BYTE with 1 channel
									  // but 841 if 2 channels parallel
									  // or 843 if 4 channels parallel
	if (_HWCH2)		buswidth=0x0841;
	waits = 0;
  ...
  /*sym - flag activates divider for symmetrical ND pulse
	...
	reg=ReadLongIOPort(drvno,0); //read LCR0 for check length 0xffffffco
...
void WriteBytePort0(UINT drvno,BYTE DWData, ULONG PortOff)
	{	// writes byte to subd with nd clks VON=h adr - VON=l data
		// PortOff: adr data 
	BOOL fResult = FALSE;
	sDLDATA WriteData;
	ULONG	DataLength;
...
{	USHORT version = 0;
	...
	if (fResult)
		...
		sprintf(wstring,"Driver LSCPCI%d",drvno);
		sprintf(pstring," version: 0x%x", version);
	...
	sprintf(pstring,"     ID = 0x%x", S0Data);
	if (MessageBox( hWnd, pstring," Board ID=53 ",MB_OK|MB_ICONEXCLAMATION ) == IDOK ) {};

	S0Data=ReadLongIOPort(drvno,0); //read LCR0 for check length 0xffffffco
	S0Data 	=  ~S0Data; //length is inverted

	sprintf(pstring,"     length = 0x%x", S0Data);
	if (MessageBox( hWnd, pstring," PCI space0 length=",MB_OK|MB_ICONEXCLAMATION ) == IDOK ) {};

	if (S0Data>=0x1F)
		{//if WE -> has space 0x20
		udata1=ReadByteS0(drvno,0x1C);
		udata2=ReadByteS0(drvno,0x1D);
		udata3=ReadByteS0(drvno,0x1E);
		udata4=ReadByteS0(drvno,0x1F);
		sprintf(pstring," ven ID = %c%c%c%c", udata1,udata2,udata3,udata4);
		if (MessageBox( hWnd, pstring," Board vendor=EBST ",MB_OK|MB_ICONEXCLAMATION ) == IDOK ) {};
		}

	if (S0Data>=0x3F)
		{//if 9056 -> has space 0x40
		S0Data=ReadLongS0(drvno,0x3C);
		sprintf(pstring,"     ID = 0x%x", S0Data);
...
val2 = ReadByteS0(drvno,0x2B) & 0x03; //dont disturb lower bits
val |= val2;
...
//	Sleep(1);
	...
	regorg = ReadByteS0(drvno,5);// get reg data
	reg = regorg | 0x20;
...
//	Sleep(1);
	...
	if (_PRGRMVON)
		{//clk with VON
		regno=4;
		regorg = ReadByteS0(drvno,regno);// get reg data
...
while (RingThreadOn==TRUE) // || FFValid(FFdrvno))
	...
	if (FFValid(Ringdrvno))
		...
		for (i=1;i<=linestofetch;i++)
			...
			if (*(pRingBuf+RingWRCnt*pixel+320)>= 3000) //wert etwa bei 1500
				{//FetchActLine=TRUE;
				ERR+=1;}
#else
...
{//start 1 write cycle to FIFO by software
	UCHAR reg=0;
	reg = ReadByteS0(drvno,11);  //enable timer
	reg |= 0x40;  
	WriteByteS0(drvno,reg,11);	
	reg = ReadByteS0(drvno,0x12); 	
	reg |= 0x40;
